...
/**
 * Analisador de Clean Code
 * Verifica princípios de código limpo e boas práticas
 */
function analyzeCleanCode(code, language = 'javascript') {
  ...
  if (language === 'javascript' || language === 'typescript') {
    try {
      ...
      // 1. Verificar código comentado (dead code)
      totalChecks++;
      ...
      // 3. Verificar funções com muitos parâmetros
      totalChecks++;
      ...
      // 5. Verificar código duplicado (DRY)
      totalChecks++;
      ...
      // 7. Verificar funções muito aninhadas
      totalChecks++;
      ...
      // 8. Verificar ausência de tratamento de erros
      totalChecks++;
      ...
      // 9. Verificar console.log em produção
      totalChecks++;
      ...
      // 10. Verificar código morto (unreachable)
      totalChecks++;
      ...
      // 11. Verificar uso de var ao invés de let/const
      totalChecks++;
      ...
      // 12. Verificar comparações não estritas
      totalChecks++;
      ...
      // 13. Verificar funções muito longas (já verificado em arquitetura, mas importante para clean code)
      totalChecks++;
      ...
      // 14. Verificar ausência de espaçamento consistente
      totalChecks++;
      ...
      // 15. Verificar comentários desnecessários
      totalChecks++;
      ...
      console.warn('Erro ao analisar clean code:', error.message);
    }
  }
...
function detectCommentedCode(code) {
  // Detecta blocos de código comentado (mais de 2 linhas)
...
function findMagicNumbers(code) {
  ...
  lines.forEach((line, index) => {
    // Ignora números em strings, comentários, e casos comuns
    if (line.trim().startsWith('//') || line.includes('/*')) return;
    
    const matches = line.matchAll(numberRegex);
    for (const match of matches) {
      const num = parseInt(match[1]);
      // Ignora anos, versões, e números comuns
...
function findUnusedVariables(ast, code) {
  // Esta é uma versão simplificada - uma implementação completa seria mais complexa
  const declared = [];
  const used = new Set();
...
function findUnreachableCode(ast) {
  const unreachable = [];
  // Implementação simplificada - detecta return seguido de código
  // Uma implementação completa seria mais complexa
  return unreachable;
}
...
function checkSpacingConsistency(code) {
  ...
  lines.forEach(line => {
    // Verifica espaços inconsistentes
